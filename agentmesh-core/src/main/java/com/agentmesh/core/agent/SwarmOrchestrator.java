...
/**
 * Swarm 编排器。
 * 通过 ExecutionMode.SWARM 启用。
 */
@Slf4j
public class SwarmOrchestrator implements AgentOrchestrator {
    ...
    public SwarmOrchestrator(AgentCollaboration collaboration,
    ...
    public Flux<StreamEvent> orchestrateStream(OrchestrationPlan plan, String input) {
        ...
        return collaboration.collaborate(agents, input, sharedContext, messageBus, collaborationId)
                ...
                .doOnError(e -> {
                    log.error("[SwarmOrchestrator] 编排异常, collaborationId={}, traceId={}",
                            collaborationId, traceId, e);
                    recordOrchestrationComplete(orchestratorType, "FAILED", sample);
                })
                .doFinally(signal -> {
                    sharedContext.clear();
                    log.info("[SwarmOrchestrator] 编排结束, collaborationId={}, signal={}",
